...
/**
 * Analisa uma string de endereço complexa como 'R1000.2', 'X0.0', 'Y5', 'D100'
 * Retorna { addressType: 'R', byteAddress: 1000, bitIndex: 2, isBit: true }
 */
function parseAddressString(str) {
  if (typeof str !== 'string') return null;
  const match = str.trim().match(/^([A-Za-z]+)(\d+)(?:\.(\d+))?$/);
  if (!match) return null;

  const typeLetter = match[1].toUpperCase();
  if (PMC_ADDRESS_TYPES[typeLetter] === undefined) return null;

  const byteAddress = parseInt(match[2], 10);
  const bitIndex = match[3] !== undefined ? parseInt(match[3], 10) : null;

  return {
    addressType: typeLetter,
    typeCode: PMC_ADDRESS_TYPES[typeLetter],
    byteAddress,
    bitIndex,
    isBit: bitIndex !== null
  };
